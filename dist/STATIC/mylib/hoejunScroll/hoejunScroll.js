...
window.HjScroll = function (TimelineMax) {
	...
	/**					Inner Utils					**/
	var renderAfter = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || function (callback) {
		window.setTimeout(callback, 0);
	};

	// ([1, 2, 3], 1) => [1, 3]
	var ipop = function ipop(arr, i) {
		var nArr = [].slice.call(arr);
		nArr.splice(i, 1);
		return nArr;
	};

	var extend = function extend(obj, obj2) {
		obj = obj || {};
		for (var key in obj2) {
	...
	/**					Construct					**/
	var HjScroll = function HjScroll(opt) {
		if (!(this instanceof HjScroll)) {
			return new HjScroll(opt);
		}
	...
	HjScroll.prototype.addTw = function (position, tw, opt) {
		...
		if (!Array.isArray(position) && !(typeof position === 'number')) {
			tw = position;position = 0.5;opt = tw;
		}
	...
	HjScroll.prototype.addProgressAnimate = function (position, tw) {
		...
		if (el && el[0]) {
			el = el[0];
		} else if (el instanceof Element) {} else {
			console.error('Element not Found');
			return;
		}
		// isFixedAnimation ?
		console.log(tw, tw.fixed);
		var calcPosition = tw.fixed ? [calcStartPosition(position, false, HjScroll.docHeight), calcDuration(position, tw.offsetTop, HjScroll.docHeight)] : [calcStartPosition(position, el), calcDuration(position, tw.offsetTop)];

		parentTw = TweenMax.to(el, calcPosition[1], {
			onUpdate: function onUpdate() {
				tw.progress(parentTw.progress());
			}
		});
	...
	HjScroll.prototype.addScrollAnimate = function (position, tw) {
		...
		// isFixedAnimation ?
		var calcPosition = tw.fixed ? [calcStartPosition([].concat(position), false, HjScroll.docHeight)] : [calcStartPosition([].concat(position), el)];
	...
	HjScroll.prototype.addFixedProgressAnimate = function (position, tw) {
		this.addProgressAnimate(position, tw, true);
	};
	HjScroll.prototype.addFixedScrollAnimate = function (position, tw) {
		this.addScrollAnimate(position, tw, true);
	};
	...
	var resizeProcess = function resizeProcess(ins) {
		...
		function reCalc(tl) {
			...
			for (var i = 0, len = tws.length; i < len; i++) {
				if (Array.isArray(tws[i].target)) {
					tempTarget = tws[i].target[0];
				} else {
					tempTarget = typeof tws[i].target === 'function' ? tws[i].target.el : tws[i].target;
				}
...
//# sourceMappingURL=hoejunScroll.js.map